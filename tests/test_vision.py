...
def test_tracker_preserves_identity_and_computes_ttc():
    ...
    tracker.update([first], timestamp=0.0, frame_width=640)

    second = Detection((150, 50, 350, 350), "car", 0.9, {"a"})
    tracker.update([second], timestamp=1.0, frame_width=640)

    assert second.track_id == first.track_id
    assert second.ttc_status == "critical_approach"
    assert second.ttc_seconds == 2.0
...
def test_tracker_measures_normalized_lateral_motion():
    ...
    tracker.update([first], timestamp=0.0, frame_width=1000)

    second = Detection((50, 100, 150, 200), "motorcycle", 0.9, {"a"})
    tracker.update([second], timestamp=1.0, frame_width=1000)
    assert second.track_id == first.track_id
